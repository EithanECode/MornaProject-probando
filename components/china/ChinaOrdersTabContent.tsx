...
function getOrderBadge(stateNum?: number) {
  ...
  if (s === 5) return { label: 'Listo p/ empaquetar', className: `${base} bg-amber-100 text-amber-800 border-amber-200` };
  ...
  if (s >= 9) return { label: 'Enviado a Vzla', className: `${base} bg-green-100 text-green-800 border-green-200` };
    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-start">
        <div className="inline-flex rounded-lg border border-slate-200 dark:border-slate-700 bg-white/70 dark:bg-slate-800/60 backdrop-blur px-1 py-1 shadow-sm">
          {(['pedidos','cajas','contenedores'] as const).map(tab => (
            <Button key={tab} variant={activeSubTab===tab? 'default':'ghost'} size="sm" onClick={()=>setActiveSubTab(tab)} className={`rounded-md ${activeSubTab===tab? 'bg-slate-900 text-white dark:bg-slate-200 dark:text-slate-900':'text-slate-700 dark:text-slate-300'}`}>{tab}</Button>
          ))}
        </div>
      </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
                          ...
                          <span className="flex items-center gap-1"><Tag className="h-3 w-3" />Cant: {p.cantidad}</span>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2">
                        {p.estado==='enviado' && (p.numericState??0) < 6 && (
                          <Button size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={()=>{ setModalEmpaquetarPedido({open:true, pedidoId: p.id}); if(boxes.length===0) fetchBoxes(); }}>Empaquetar</Button>
                        )}
                        {p.estado==='enviado' && (p.numericState??0) >=6 && (
                          <Button variant="outline" size="sm" disabled={(p.numericState??0)>=9} onClick={()=>{ if((p.numericState??0)<9) handleUnpackOrder(p.id); }}>Desempaquetar</Button>
                        )}
                        <Button variant="outline" size="sm" onClick={()=>{ if(p.pdfRoutes){ const bust=p.pdfRoutes.includes('?')?`&t=${Date.now()}`:`?t=${Date.now()}`; window.open(p.pdfRoutes+bust,'_blank','noopener,noreferrer'); } else toast({ title:'Sin PDF'}); }}><Eye className="h-4 w-4" /></Button>
                        {p.estado==='pendiente' ? (
      ...
      {activeSubTab==='cajas' && (
        <Card className="bg-white/80 dark:bg-slate-800/70 backdrop-blur-sm border-slate-200 dark:border-slate-700">
          ...
          <CardContent>
            {boxes.length===0 ? (<div className="py-10 text-center text-sm">No hay cajas.</div>) : boxes.filter((b,idx)=>{ if(!filtroCaja) return true; const id=b.box_id ?? b.boxes_id ?? b.id ?? idx; return String(id).toLowerCase().includes(filtroCaja.toLowerCase()); }).length===0 ? (<div className="py-10 text-center text-sm">Sin coincidencias.</div>) : (
              <div className="space-y-3">
                {boxes.filter((b,idx)=>{ if(!filtroCaja) return true; const id=b.box_id ?? b.boxes_id ?? b.id ?? idx; return String(id).toLowerCase().includes(filtroCaja.toLowerCase()); }).map((box,idx)=>{ const id=box.box_id ?? box.boxes_id ?? box.id ?? idx; const created=box.creation_date ?? box.created_at ?? ''; const stateNum=(box.state??1) as number; const countKey = box.box_id ?? box.boxes_id ?? box.id ?? id; const badge=getBoxBadge(stateNum); return (
                  <div key={id as any} className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 border border-slate-200 dark:border-slate-600">
                    ...
                    <div className="flex items-center gap-3">
                      <Badge className={badge.className}>{badge.label}</Badge>
                      {stateNum===1 && (<Button size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={()=>{ const currentBoxId=box.box_id ?? box.boxes_id ?? box.id; setModalEmpaquetarCaja({open:true, boxId: currentBoxId}); if(containers.length===0) fetchContainers(); }}>Empaquetar</Button>)}
                      {stateNum===2 && (<Button variant="outline" size="sm" onClick={()=>{ const currentBoxId=box.box_id ?? box.boxes_id ?? box.id; if(currentBoxId!==undefined) handleUnpackBox(currentBoxId as any); }}>Desempaquetar</Button>)}
                      {stateNum>=3 && (<Button variant="outline" size="sm" disabled>Desempaquetar</Button>)}
                      <Button variant="outline" size="sm" onClick={()=>{ const boxId=box.box_id ?? box.boxes_id ?? box.id; setModalVerPedidosCaja({open:true, boxId}); if(boxId!==undefined) fetchOrdersByBoxId(boxId); }}>Ver pedidos</Button>
                      <Button variant="outline" size="sm" className="text-red-600 border-red-300 hover:bg-red-50 disabled:opacity-50" disabled={(box.state??1)>=3} onClick={()=>{ if((box.state??1)>=3){ toast({ title:'No permitido', description:'Caja enviada'}); return;} setModalEliminarCaja({open:true, box}); }}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                ); })}
              </div>
            )}
            {boxesLoading && <p className="text-center text-sm mt-4">Cargando cajas...</p>}
          </CardContent>
        </Card>
      )}
      ...
      {activeSubTab==='contenedores' && (
        <Card className="bg-white/80 dark:bg-slate-800/70 backdrop-blur-sm border-slate-200 dark:border-slate-700">
          ...
          <CardContent>
            {containers.length===0 ? (<div className="py-10 text-center text-sm">No hay contenedores.</div>) : containers.filter((c,idx)=>{ if(!filtroContenedor) return true; const id=c.container_id ?? c.containers_id ?? c.id ?? idx; return String(id).toLowerCase().includes(filtroContenedor.toLowerCase()); }).length===0 ? (<div className="py-10 text-center text-sm">Sin coincidencias.</div>) : (
              <div className="space-y-3">
                {containers.filter((c,idx)=>{ if(!filtroContenedor) return true; const id=c.container_id ?? c.containers_id ?? c.id ?? idx; return String(id).toLowerCase().includes(filtroContenedor.toLowerCase()); }).map((container,idx)=>{ const id=container.container_id ?? container.containers_id ?? container.id ?? idx; const created=container.creation_date ?? container.created_at ?? ''; const stateNum=(container.state??1) as number; const badge=getContainerBadge(stateNum); return (
                  <div key={id as any} className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 border border-slate-200 dark:border-slate-600">
                    ...
                    <div className="flex items-center gap-3">
                      <Badge className={badge.className}>{badge.label}</Badge>
                      <Button size="sm" className="bg-blue-600 hover:bg-blue-700 text-white" disabled={stateNum!==2} onClick={()=>handleSendContainer(container)}><Truck className="h-4 w-4" />Enviar</Button>
                      <Button variant="outline" size="sm" onClick={()=>{ const containerId = container.container_id ?? container.containers_id ?? container.id; setModalVerCajasCont({open:true, containerId}); if(containerId!==undefined) fetchBoxesByContainerId(containerId); }}>Ver cajas</Button>
                      <Button variant="outline" size="sm" className="text-red-600 border-red-300 hover:bg-red-50 disabled:opacity-50" disabled={(container.state??1)>=3} onClick={()=>{ if((container.state??1)>=3){ toast({ title:'No permitido', description:'Contenedor enviado'}); return;} setModalEliminarContenedor({open:true, container}); }}><Trash2 className="h-4 w-4" /></Button>
                    </div>
                  </div>
                ); })}
              </div>
            )}
            {containersLoading && <p className="text-center text-sm mt-4">Cargando contenedores...</p>}
          </CardContent>
        </Card>
      )}
      ...
      {modalVerPedidosCaja.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-3xl mx-4 w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold text-slate-900 dark:text-white">Pedidos de la caja #BOX-{String(modalVerPedidosCaja.boxId ?? '')}</h3><Button variant="ghost" size="sm" onClick={()=>setModalVerPedidosCaja({open:false})} className="h-8 w-8 p-0">✕</Button></div>
            {ordersByBoxLoading ? <p className="text-center text-sm text-slate-500 py-6">Cargando...</p> : ordersByBox.length===0 ? (
              <div className="text-center py-12"><Package className="h-12 w-12 text-slate-400 mx-auto mb-4" /><p className="text-slate-500 dark:text-slate-400">Sin pedidos en esta caja.</p></div>
            ) : (
              <div className="space-y-3">
      ...
      {modalVerCajasCont.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalVerCajasContRef} className="bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-3xl mx-4 w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold text-slate-900 dark:text-white">Cajas del contenedor #CONT-{String(modalVerCajasCont.containerId ?? '')}</h3><Button variant="ghost" size="sm" onClick={()=>setModalVerCajasCont({open:false})} className="h-8 w-8 p-0">✕</Button></div>
            {boxesByContainerLoading ? <p className="text-center text-sm text-slate-500 py-6">Cargando...</p> : boxesByContainer.length===0 ? (
              <div className="text-center py-12"><Boxes className="h-12 w-12 text-slate-400 mx-auto mb-4" /><p className="text-slate-500 dark:text-slate-400">Sin cajas aún.</p></div>
            ) : (
              <div className="space-y-3">
                {boxesByContainer.map((box,idx)=>{ const id = box.box_id ?? box.boxes_id ?? box.id ?? idx; const created = box.creation_date ?? box.created_at ?? ''; const stateNum=(box.state ?? 1) as number; const badge=getBoxBadge(stateNum); return (
                  <div key={id as any} className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 border border-slate-200 dark:border-slate-600">
                    ...
                    <div className="flex items-center gap-3">
                      <Badge className={badge.className}>{badge.label}</Badge>
                      {stateNum===2 && (<Button variant="outline" size="sm" onClick={()=>{ const boxId=box.box_id ?? box.boxes_id ?? box.id; const containerId=modalVerCajasCont.containerId; handleUnpackBox(boxId as any,{ containerId}); }}>Desempaquetar</Button>)}
                      {stateNum>=3 && (<Button variant="outline" size="sm" disabled>Desempaquetar</Button>)}
                      <Button variant="outline" size="sm" onClick={()=>{ const boxId=box.box_id ?? box.boxes_id ?? box.id; setModalVerPedidosCaja({open:true, boxId}); if(boxId!==undefined) fetchOrdersByBoxId(boxId); }}>Ver pedidos</Button>
                    </div>
                  </div>
      ...
      {modalCotizar.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalCotizarRef} className={`bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-lg mx-4 w-full transition-all ${isClosingModalCotizar? 'scale-95 opacity-0' : 'scale-100 opacity-100'} duration-200`}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold text-slate-900 dark:text-white">Cotizar Pedido</h3><Button variant="ghost" size="sm" onClick={closeModalCotizar} className="h-8 w-8 p-0">✕</Button></div>
            <form onSubmit={e=>{ e.preventDefault(); const precio=Number((e.target as any).precio.value); if(precio>0 && modalCotizar.pedido) cotizarPedido(modalCotizar.pedido, precio); }} className="space-y-4">
              <div className="grid grid-cols-2 gap-4 text-sm"><div><p className="font-medium">Cliente</p><p>{modalCotizar.pedido?.cliente}</p></div><div><p className="font-medium">Producto</p><p>{modalCotizar.pedido?.producto}</p></div><div><p className="font-medium">Cantidad</p><p>{modalCotizar.pedido?.cantidad}</p></div><div><p className="font-medium">Estado</p><p>{modalCotizar.pedido?.estado}</p></div></div>
              <div><label className="text-sm font-medium">Precio unitario</label><div className="relative mt-1"><span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500">$</span><input name="precio" type="number" required min="0" step="0.01" defaultValue={modalCotizar.precioCotizacion} onChange={e=>setModalCotizar(prev=>({...prev, precioCotizacion:Number(e.target.value)}))} className="w-full pl-7 pr-3 py-2 rounded-md border border-slate-300 bg-white dark:bg-slate-800" /></div></div>
              <div className="text-sm"><p className="font-medium">Total</p><p className="text-green-600 dark:text-green-400 font-semibold text-lg">${((modalCotizar.precioCotizacion||0)*(modalCotizar.pedido?.cantidad||0)).toLocaleString()}</p></div>
              <div className="flex justify-end gap-2"><Button type="button" variant="outline" onClick={closeModalCotizar}>Cancelar</Button><Button type="submit" className="bg-blue-600 hover:bg-blue-700">Guardar</Button></div>
            </form>
          </div>
        </div>
      )}
      ...
      {modalEmpaquetarPedido.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalEmpaquetarPedidoRef} className={`bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-2xl mx-4 w-full max-h-[85vh] overflow-y-auto transition-all ${isClosingModalEmpaquetarPedido? 'scale-95 opacity-0' : 'scale-100 opacity-100'} duration-200`}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold text-slate-900 dark:text-white">Seleccionar caja para el pedido #ORD-{modalEmpaquetarPedido.pedidoId}</h3><Button variant="ghost" size="sm" onClick={closeModalEmpaquetarPedido} className="h-8 w-8 p-0">✕</Button></div>
            {boxesLoading ? (<p className="text-center text-sm py-6">Cargando cajas...</p>) : boxes.length===0 ? (<div className="text-center py-12"><Boxes className="h-12 w-12 text-slate-400 mx-auto mb-4" /><p className="text-sm text-slate-500">No hay cajas disponibles.</p></div>) : (
              <div className="space-y-3">{boxes.map((box,idx)=>{ const id=box.box_id ?? box.boxes_id ?? box.id ?? idx; const created=box.creation_date ?? box.created_at ?? ''; const stateNum=(box.state??1) as number; return (
                <div key={id as any} className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 border border-slate-200 dark:border-slate-600">
                  <div className="space-y-1"><p className="font-semibold text-slate-900 dark:text-white">#BOX-{id}</p><p className="text-xs text-slate-500 dark:text-slate-400">{created? new Date(created).toLocaleString('es-ES'):'—'}</p></div>
                  <div className="flex items-center gap-3"><Badge className={`border ${stateNum===1?'bg-blue-100 text-blue-800 border-blue-200': stateNum===2? 'bg-green-100 text-green-800 border-green-200':'bg-gray-100 text-gray-800 border-gray-200'}`}>{stateNum===1?'Nueva': stateNum===2? 'Empaquetada':`Estado ${stateNum}`}</Badge><Button size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={()=> modalEmpaquetarPedido.pedidoId && handleSelectCajaForPedido(modalEmpaquetarPedido.pedidoId, box)}>Seleccionar</Button></div>
                </div>
              ); })}</div>
            )}
          </div>
      ...
      {modalCrearCaja.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalCrearCajaRef} className={`bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md mx-4 w-full transition-all ${isClosingModalCrearCaja? 'scale-95 opacity-0':'scale-100 opacity-100'} duration-200`}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold">Crear caja</h3><Button variant="ghost" size="sm" onClick={closeModalCrearCaja} className="h-8 w-8 p-0">✕</Button></div>
            <p className="text-sm mb-6">¿Deseas crear una nueva caja?</p>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={closeModalCrearCaja} disabled={creatingBox}>Cancelar</Button><Button className="bg-blue-600 hover:bg-blue-700" disabled={creatingBox} onClick={handleConfirmCrearCaja}>{creatingBox?'Creando...':'Aceptar'}</Button></div>
          </div>
        </div>
      )}
      ...
      {modalEliminarCaja.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalEliminarCajaRef} className={`bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md mx-4 w-full transition-all ${isClosingModalEliminarCaja? 'scale-95 opacity-0':'scale-100 opacity-100'} duration-200`}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold">Eliminar caja</h3><Button variant="ghost" size="sm" onClick={closeModalEliminarCaja} className="h-8 w-8 p-0">✕</Button></div>
            <p className="text-sm mb-6">¿Seguro que deseas eliminar la caja #BOX-{String(modalEliminarCaja.box?.box_id ?? modalEliminarCaja.box?.boxes_id ?? modalEliminarCaja.box?.id ?? '')}? Esta acción no se puede deshacer.</p>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={closeModalEliminarCaja} disabled={deletingBox}>Cancelar</Button><Button className="bg-red-600 hover:bg-red-700" disabled={deletingBox} onClick={handleDeleteCaja}>{deletingBox? 'Eliminando...':'Eliminar'}</Button></div>
          </div>
        </div>
      )}
      ...
      {modalEmpaquetarCaja.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalEmpaquetarCajaRef} className={`bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-2xl mx-4 w-full max-h-[85vh] overflow-y-auto transition-all ${isClosingModalEmpaquetarCaja? 'scale-95 opacity-0':'scale-100 opacity-100'} duration-200`}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold">Seleccionar contenedor para la caja #BOX-{modalEmpaquetarCaja.boxId}</h3><Button variant="ghost" size="sm" onClick={closeModalEmpaquetarCaja} className="h-8 w-8 p-0">✕</Button></div>
            {containersLoading ? (<p className="text-center text-sm py-6">Cargando contenedores...</p>) : containers.length===0 ? (<div className="py-12 text-center"><Boxes className="h-12 w-12 text-slate-400 mx-auto mb-4" /><p className="text-sm">No hay contenedores.</p></div>) : (
              <div className="space-y-3">{containers.map((container,idx)=>{ const id=container.container_id ?? container.containers_id ?? container.id ?? idx; const created = container.creation_date ?? container.created_at ?? ''; const stateNum=(container.state??1) as number; return (
                <div key={id as any} className="flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 border border-slate-200 dark:border-slate-600">
                  ...
                  <div className="flex items-center gap-3"><Badge className={getContainerBadge(stateNum).className}>{getContainerBadge(stateNum).label}</Badge><Button size="sm" className="bg-indigo-600 hover:bg-indigo-700" onClick={()=> modalEmpaquetarCaja.boxId && handleSelectContenedorForCaja(modalEmpaquetarCaja.boxId, container)}>Seleccionar</Button></div>
                </div>
              ); })}</div>
            )}
          </div>
      ...
      {modalCrearContenedor.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalCrearContenedorRef} className={`bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md mx-4 w-full transition-all ${isClosingModalCrearContenedor? 'scale-95 opacity-0':'scale-100 opacity-100'} duration-200`}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold">Crear contenedor</h3><Button variant="ghost" size="sm" onClick={closeModalCrearContenedor} className="h-8 w-8 p-0">✕</Button></div>
            <p className="text-sm mb-6">¿Deseas crear un nuevo contenedor?</p>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={closeModalCrearContenedor} disabled={creatingContainer}>Cancelar</Button><Button className="bg-blue-600 hover:bg-blue-700" disabled={creatingContainer} onClick={handleConfirmCrearContenedor}>{creatingContainer? 'Creando...':'Aceptar'}</Button></div>
          </div>
        </div>
      )}
      ...
      {modalEliminarContenedor.open && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50">
          <div ref={modalEliminarContenedorRef} className={`bg-white dark:bg-slate-900 rounded-2xl p-6 max-w-md mx-4 w-full transition-all ${isClosingModalEliminarContenedor? 'scale-95 opacity-0':'scale-100 opacity-100'} duration-200`}>
            <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-bold">Eliminar contenedor</h3><Button variant="ghost" size="sm" onClick={closeModalEliminarContenedor} className="h-8 w-8 p-0">✕</Button></div>
            <p className="text-sm mb-6">¿Seguro que deseas eliminar el contenedor #CONT-{String(modalEliminarContenedor.container?.container_id ?? modalEliminarContenedor.container?.containers_id ?? modalEliminarContenedor.container?.id ?? '')}? Esta acción no se puede deshacer.</p>
            <div className="flex justify-end gap-2"><Button variant="outline" onClick={closeModalEliminarContenedor} disabled={deletingContainer}>Cancelar</Button><Button className="bg-red-600 hover:bg-red-700" disabled={deletingContainer} onClick={handleDeleteContenedor}>{deletingContainer? 'Eliminando...':'Eliminar'}</Button></div>
          </div>
        </div>
      )}
  </div>

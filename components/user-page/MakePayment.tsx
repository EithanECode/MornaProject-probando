...
interface PaymentMethods {
    [key: string]: {
        title: string;
        options: {
            [key: string]: PaymentMethodOption;
        };
    };
}
...
function MakePayment({ orderId, totalAmount, onPaymentConfirmed }: MakePaymentProps) {
    ...
    function getPaymentInstructions(typeKey: string, selectedPaymentMethod: string): string | undefined {
        if (
            paymentMethods[typeKey] &&
            paymentMethods[typeKey].options[selectedPaymentMethod]
        ) {
            return paymentMethods[typeKey].options[selectedPaymentMethod].instructions;
        }
        return undefined;
                    ...
                    <form onSubmit={handleSubmitPayment} className="payment-proof-form">
                        ...
                        >
                            ...
                            <input
                                ...
                                required
                            />
                            {paymentProof && <span className="file-name">{(paymentProof as File).name}</span>}

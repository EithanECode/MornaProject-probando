          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {loading ? (
              ...
              <Card className="bg-white/80 backdrop-blur-sm border-slate-200">
                ...
                {/* Layout Tablet */}
                <div className={`hidden md:flex lg:hidden min-h-screen overflow-x-hidden ${
                  theme === 'dark' ? 'bg-slate-900' : 'bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50'
                }`}>
                  <Sidebar 
                    isExpanded={sidebarExpanded} 
                  ...
                  <main className="w-full">
                    ...
                    <div className="p-4 space-y-6">
                      ...
                      <div className="bg-gradient-to-r from-orange-600 to-red-600 rounded-xl p-6 text-white">
                        <div className="flex flex-col gap-4">
                          <div>
                            <h1 className="text-xl font-bold">Revisar Pedidos</h1>
                            <p className="text-orange-100 mt-1 text-sm">Revisa y envía pedidos a China para cotización</p>
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div className="text-center">
                              <p className="text-xl font-bold">{orders.filter(o => o.state === 1).length}</p>
                              <p className="text-xs text-orange-100">PENDIENTES</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xl font-bold">{orders.filter(o => o.state === 2).length}</p>
                              <p className="text-xs text-orange-100">REVISANDO</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xl font-bold">{orders.filter(o => o.state === 3).length}</p>
                              <p className="text-xs text-orange-100">COTIZADOS</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xl font-bold">{orders.filter(o => o.state === 4).length}</p>
                              <p className="text-xs text-orange-100">PROCESANDO</p>
                            </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 gap-4">
                        {loading ? (
                          <Card className="bg-white/80 backdrop-blur-sm border-slate-200">
                            <CardContent className="p-8 text-center">Cargando pedidos...</CardContent>
                          </Card>
                        ) : error ? (
                          <Card className="bg-white/80 backdrop-blur-sm border-slate-200">
                          ...
                          <Card className="bg-white/80 backdrop-blur-sm border-slate-200">
                            <CardContent className="p-8 text-center">
                              ...
                              <h3 className="text-base font-semibold text-slate-900 mb-2">No hay pedidos para revisar</h3>
                              <p className="text-sm text-slate-600">Todos los pedidos han sido procesados o no hay coincidencias con los filtros.</p>
                            </CardContent>
                          </Card>
                        ) : (
                          sortedOrders.map((order) => (
                              ...
                              <CardHeader>
                                <div className="flex items-center justify-between">
                                  <div>
                                    ...
                                    <p className="text-xs text-slate-600">{order.id} - {order.clientName}</p>
                                  </div>
                                  <div className="flex gap-1">
                                    {order.state === 1 && (
                                      <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200 text-xs">PENDIENTE</Badge>
                                    )}
                                    {order.state === 2 && (
                                      <Badge className="bg-green-100 text-green-800 border-green-200 text-xs">REVISANDO</Badge>
                                    )}
                                    {order.state === 3 && (
                                      <Badge className="bg-purple-100 text-purple-800 border-purple-200 text-xs">COTIZADO</Badge>
                                    )}
                                    {order.state === 4 && (
                                      <Badge className="bg-blue-100 text-blue-800 border-blue-200 text-xs">PROCESANDO</Badge>
                                    )}
                                  </div>
                                </div>
                              </CardHeader>

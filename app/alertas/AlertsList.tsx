...
const renderAlertSection = (title: string, alerts: Alert[], color: string, onResolve: (id: string) => void, onView: (alert: Alert) => void) => {
  if (alerts.length === 0) return null;
  return (
    <div className={`bg-white rounded-xl shadow-sm border border-${color}-200 overflow-hidden mb-6`}>
      <div className={`px-6 py-4 border-b border-${color}-200 bg-${color}-50`}>
...
const AlertsList: React.FC<AlertsListProps> = ({ alerts, onResolve, onView }) => {
  ...
  return (
    <>
      {renderAlertSection('Alertas Críticas', criticalAlerts, 'red', onResolve, onView)}

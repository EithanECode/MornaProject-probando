...
export default function AjustesPage() {
  ...
  const getUserRole = () => {
    ...
    if (pathname.includes('/venezuela')) return 'venezuela';
    if (pathname.includes('/china')) return 'china';
  ...
  // Estados de notificaciones (específicas por rol)
  const [notifications, setNotifications] = useState({
    emailNotifications: true,
    pushNotifications: true,
    alertasCriticas: userRole === 'admin' || userRole === 'venezuela',
    reportesDiarios: userRole === 'admin',
    actualizacionesSistema: true,
    notificacionesMarketing: userRole === 'cliente',
    notificacionesPedidos: userRole === 'cliente' || userRole === 'venezuela',
    notificacionesCotizaciones: userRole === 'china'
  });
      ...
      <main className={`transition-all duration-300 flex-1 ${
        ...
        <header className={mounted && theme === 'dark' ? 'bg-slate-800/80 backdrop-blur-sm border-b border-slate-700 sticky top-0 z-40' : 'bg-white/80 backdrop-blur-sm border-b border-slate-200 sticky top-0 z-40'}>
          <div className="px-4 md:px-5 lg:px-6 py-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className={`text-xl md:text-2xl lg:text-3xl font-bold ${mounted && theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>Ajustes</h1>
        ...
        <div className="p-4 md:p-5 lg:p-6 space-y-6 md:space-y-8">
          <Tabs defaultValue="perfil" className="space-y-6">
            <TabsList className="grid w-full grid-cols-1 md:grid-cols-4 lg:w-auto lg:grid-cols-4">
              ...
              <Card className="bg-white/80 backdrop-blur-sm border-slate-200 dark:bg-slate-800/80 dark:border-slate-700">
                ...
                <CardContent className="space-y-6">
                  <div className="space-y-4">
                    ...
                    <Separator />
                    {(userRole === 'admin' || userRole === 'venezuela') && (
                      <>
                        <div className="flex items-center justify-between">
                          <div className="space-y-1">
                    ...
                    {(userRole === 'venezuela' || userRole === 'cliente') && (
                      <div className="flex items-center justify-between">
                        <div className="space-y-1">
                          <Label>Notificaciones de pedidos</Label>
                          <p className="text-sm text-slate-600 dark:text-slate-400">
